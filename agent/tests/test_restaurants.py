"""Restaurant availability: the deterministic mock provider and the function tool."""
...
from voice_agent.tools.restaurants.tool import AvailabilityTool
...
def test_tool_output_is_compact_and_says_not_booked() -> None:
    result = RestaurantAvailability(
        ...
        party_size=2,
        status="alternatives",
        slots=[TimeSlot(time=t) for t in ("17:00", "19:00", "19:45", "20:00", "21:30")],
        provider="test",
    )
...
def test_tool_output_lists_each_time_once() -> None:
    result = RestaurantAvailability(
        ...
        party_size=2,
        status="available",
        slots=[TimeSlot(time="19:30", area=a) for a in ("bar", "patio")] + [TimeSlot(time="20:00")],
        provider="test",
    )
...
def _tool(provider: ReservationProvider) -> AvailabilityTool:
    return build_restaurant_availability_tool(provider, clock=lambda: TODAY)


async def test_decorated_tool_is_directly_callable() -> None:
    tool = _tool(MockReservationProvider())
    raw = await tool(restaurant="Nopa", date="2026-10-02", time="19:30", party_size=4)
    ...
    assert raw == await tool(restaurant="Nopa", date="2026-10-02", time="19:30", party_size=4)
