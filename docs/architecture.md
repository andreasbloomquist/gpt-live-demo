...
> system), [`getting-started.md`](getting-started.md) (run it).
...
flowchart LR
    subgraph Client["Caller's device"]
        UI["Next.js page<br/>frontend/app/page.tsx"]
    end
...
Four hops, each chosen deliberately:
...
- **Role.** A small Next.js (App Router) page. It fetches a token, joins the room with
  `<LiveKitRoom>`, plays the agent's audio, shows the agent state (`useVoiceAssistant`), and
  renders a live transcript from `useTranscriptions()`.
- **Why.** The smallest UI that still shows everything a voice agent needs: connect, mic,
  visualizer, transcript, hang up. LiveKit's React components do the WebRTC work.
- **Benefit.** You can read the whole client in a few minutes. The agent itself is not
  browser-specific.
- **Drawback.** It's a demo UI. It has no auth, no rate limiting on the token route, and no
  reconnection UX beyond what the components do by default.
...
- **Benefit.** Secrets stay server-side. Every browser session gets its own room and its own
  agent job.
- **Drawback.** Anyone who can reach the route gets a token, and every token starts a paid
  GPT-Live session. Put your app's auth and a rate limit in front of it before you ship.
...
- **Benefit.** The entrypoint is about 50 lines of real code. There is no VAD, STT, TTS, or turn
  detector to configure, because GPT-Live does all of that.
...
- **Why.** Missing keys should fail at session start with an actionable message, not at import.
  Any transcript should be traceable to the exact prompt that produced it.
...
- **Drawback.** There are no metrics or tracing exporters wired up. It's logs only.
...
|---|---|---|
| `agent/voice_agent/main.py` | `AgentServer`, per-session entrypoint, default agent name, trace attributes, usage logging, `cli()` console script. | Yes |
...
| `agent/voice_agent/config.py` | `Settings` from env/`.env`; `require_openai_api_key()`. | No |
...
| `frontend/` | Next.js client and token route. | Client SDK |
...
| `OPENAI_API_KEY` | none | `model.py` | Needs GPT-Live access. If missing, `ConfigurationError` is raised at session start. |
...
| `AGENT_TIMEZONE` | `America/Los_Angeles` | `runtime.py`, restaurant tool | IANA name. Unknown names fall back to UTC with a warning. |
...
The frontend has its own file, `frontend/.env.local`, copied from `frontend/.env.example`:
...
| `OPENAI_API_KEY` missing | `ConfigurationError` in `build_gpt_live_model` | Worker log; the browser shows "Waiting for agent…" |
| `RESTAURANT_PROVIDER=opentable` without credentials | `ConfigurationError` in `resolve_tools` | Worker log at session start. |
