...
from .storage import SQLiteCallRepository
    ...
    try:
        ...
        for path in files:
            ...
            if outcome == "conflict":
                ...
                errors += 1
                continue
            if outcome == "duplicate" and args.reanalyze:
...
def main(argv: list[str] | None = None) -> int:
    ...
    seed.add_argument("--provider", choices=["auto", "openai", "heuristic"])
    seed.add_argument(
        "--reanalyze", action="store_true", help="re-run analysis for records already stored"
    )
    seed.set_defaults(func=lambda a: asyncio.run(_seed(a)))
    ...
    analyze.add_argument("--provider", choices=["auto", "openai", "heuristic"])
    analyze.set_defaults(func=lambda a: asyncio.run(_analyze(a)))
