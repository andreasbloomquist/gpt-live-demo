...
from evals.tests.conftest import EditFile, commit_all
...
def plan_for(repo: Path, overrides: Overrides | None = None, *, fast_path: bool = True) -> Plan:
    """Plan the fixture repo's working tree against its last commit."""
    return build_plan(repo, "HEAD", None, overrides=overrides or Overrides(), fast_path=fast_path)
...
def test_whitespace_and_comment_only_prompt_edit_runs_nothing(repo: Path, edit: EditFile) -> None:
...
def test_backend_prompt_runs_brain_and_voice(repo: Path, edit: EditFile) -> None:
    edit(TOOL_POLICY, lambda t: t.rstrip() + "\n\nPrefer the most recent sources.\n")
    assert planned(plan_for(repo)) == every("brain") | every("voice")


def test_tool_schema_change_runs_brain_and_voice(repo: Path, edit: EditFile) -> None:
...
def test_tool_impl_change_runs_brain_for_suites_using_that_tool(repo: Path, edit: EditFile) -> None:
    edit(
        MOCK_PROVIDER,
        lambda t: t.replace(
...
def test_tool_impl_comment_and_docstring_edit_runs_nothing(repo: Path, edit: EditFile) -> None:
    edit(
        MOCK_PROVIDER,
        lambda t: t.replace(
            "def _to_minutes(value: dt.time) -> int:\n",
...
def test_voice_setting_default_runs_voice_only(repo: Path, edit: EditFile) -> None:
    edit(
...
def test_backend_setting_default_runs_both(repo: Path, edit: EditFile) -> None:
    edit(
        "agent/voice_agent/config.py",
...
def test_secret_setting_change_runs_nothing(repo: Path, edit: EditFile) -> None:
    edit(
        "agent/voice_agent/config.py",
...
def test_lockfile_bump_of_livekit_runs_everything(repo: Path, edit: EditFile) -> None:
    edit(
        "uv.lock",
        lambda t: t.replace(
            'name = "livekit-agents"\nversion = "1.8.3"',
...
def test_suite_edit_runs_only_that_suite(repo: Path, edit: EditFile) -> None:
...
def test_suite_comment_edit_runs_nothing(repo: Path, edit: EditFile) -> None:
    edit("evals/suites/web_search.yaml", lambda t: "# reviewed\n" + t)
    assert planned(plan_for(repo)) == set()


def test_voice_runner_change_runs_voice_only(repo: Path, edit: EditFile) -> None:
    edit("evals/runners/voice.py", lambda t: t.replace("SETTLE_S = 2.5", "SETTLE_S = 3.0"))
    assert planned(plan_for(repo)) == every("voice")


def test_composer_change_runs_everything(repo: Path, edit: EditFile) -> None:
...
def test_head_as_commit_ref(repo: Path, edit: EditFile) -> None:
    base = repo / ".git"
    assert base.exists()
...
def test_backend_options_in_model_py_run_both_tiers(repo: Path, edit: EditFile) -> None:
...
def test_voice_only_runtime_change_runs_voice_only(repo: Path, edit: EditFile) -> None:
...
def test_module_imported_by_another_tool_reruns_that_tools_suites(
    repo: Path, edit: EditFile
) -> None:
...
def test_tool_claiming_config_does_not_hide_settings_changes(repo: Path, edit: EditFile) -> None:
    edit(
...
def test_import_time_print_does_not_break_fingerprinting(repo: Path, edit: EditFile) -> None:
